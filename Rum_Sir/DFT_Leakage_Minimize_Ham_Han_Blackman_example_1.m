...
x_sig_Hamming = x_Hamming';
 X_dft_Hamming = dft(x_sig_Hamming, N);     % DFT of x_sig_Hamming

 figure(5); 
stem(mf,X_dft_Hamming,'LineStyle','-',...
     'MarkerSize',15,'Marker','s',...
     'MarkerFaceColor','black',...
     'MarkerEdgeColor','green')
grid on;
title('Magnitude of X_dft_Hamming')
xlabel('Frequency m (KHz)')
ylabel('Magnitude')
...
x_sig_Hanning = x_Hanning';
 X_dft_Hanning = dft(x_sig_Hanning, N);     % DFT of x_sig_Hanning

 figure(8); 
stem(mf,X_dft_Hanning,'LineStyle','-',...
     'MarkerSize',15,'Marker','s',...
     'MarkerFaceColor','black',...
     'MarkerEdgeColor','green')
grid on;
title('Magnitude of X_dft_Hanning')
xlabel('Frequency m (KHz)')
ylabel('Magnitude')
...
x_sig_Blackman = x_Blackman';
 X_dft_Blackman = dft(x_sig_Blackman, N);     % DFT of x_sig_Blackman

 figure(11); 
stem(mf,X_dft_Blackman,'LineStyle','-',...
     'MarkerSize',15,'Marker','s',...
     'MarkerFaceColor','black',...
     'MarkerEdgeColor','green')
grid on;
title('Magnitude of X_dft_Blackman')
xlabel('Frequency m (KHz)')
ylabel('Magnitude')
 
% Ploting DFT outputs combined in one plot
figure(12); 
plot(mf,Dft_x_comb,'k--o');   grid on;    hold on;
plot(mf,X_dft_Hamming,'b--o');    hold on;  
plot(mf,X_dft_Hanning,'r--s');    hold on;  
plot(mf,X_dft_Blackman,'c--o');
stem(mf,Dft_x_comb,'LineStyle','-',...
     'MarkerSize',15,'Marker','s',...
     'MarkerFaceColor','black',...
     'MarkerEdgeColor','green')
